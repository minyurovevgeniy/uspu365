...
//	include("./php/loadData.php");
	...
	$backUrl='<a href="loadSPOexamTimetable.php">Попробуйте ещё раз</a><br>';
	...
	{
		$stmt=$pdo->prepare("TRUNCATE TABLE spo_exam_timetable");
		$stmt->execute();
	}
	...
	for($rowNumber=$MIN_ROW;$rowNumber<=$MAX_ROW;$rowNumber++)
	{
		...
		//$stmt=$pdo->prepare("SELECT COUNT(*) FROM spo_exam_timetable WHERE spo_exam_date=? AND spo_exam_start_time=? AND spo_exam_end_time=?");
		//$stmt->execute(array($dateToWrite,$startTimeToWrite,$endTimeToWrite));
		//$row=$stmt->fetch(PDO::FETCH_LAZY);
		//if ($row['COUNT(*)']<=0)
		//{
			$stmt=$pdo->prepare("INSERT INTO spo_exam_timetable SET spo_exam_date=?, spo_exam_start_time=?, spo_exam_end_time=?, spo_exam_notes=?, spo_exam_room=?, is_cancelled=?");
	...
	$timetable=array();
  $stmt=$pdo->prepare("SELECT * FROM spo_exam_timetable ORDER BY spo_exam_date ASC, spo_exam_start_time ASC");
  $stmt->execute();
    ...
    if ($shouldDisplay>0)
    {
      ...
      $timetable['lessons'][]=
        ...
        (
          ...
          'date'=>$reversedDate,
          'weekday'=>$weekday,
          'start_time'=>$row['spo_exam_start_time'],
          'end_time'=>$row['spo_exam_end_time'],
...
*/
	...
	echo '<a href="loadSPOexamTimetable.php">Загрузить ещё расписание</a><br>';
?>
