<?php
	...
	foreach($profileId as $value)
	{
		$copiesCount=0;
		$originalCount=0;
		$stmt=$pdo->prepare("SELECT abiturient, diploma_type FROM magistracy_requests WHERE profile=? ORDER BY score DESC");
		...
		$stmt=$pdo->prepare("SELECT special FROM magistracy_requests WHERE profile=? AND abiturient=?");
		$stmt->execute(array($value,$abiturientId));
		$special=$stmt->fetch(PDO::FETCH_LAZY);
		
		$stmt=$pdo->prepare("SELECT faculty, budget, form, outbox_education, profile_description FROM magistracy_profiles WHERE profile_id=?");
		$stmt->execute(array($value));
		...
		(
			...
			"profile"=>iconv("cp1251","utf-8",$magisterProfileNameById[$extra['profile_description']]),
