<?php
	...
	if ($_POST['password']!="3ChZCpTa6G6nRkv")
	{
		die("Неверный пароль");
	}
	...
	$stmt=$pdo->prepare("DELETE FROM bachelor_announcements WHERE announcement_id=?");
	...
	$stmt=$pdo->prepare("SELECT * FROM bachelor_announcements ORDER BY announcement_time DESC LIMIT 5");
	$stmt->execute();
	$response=array();
	while($row=$stmt->fetch(PDO::FETCH_LAZY))
	{
	...
	$myfile = fopen("./announcements_bachelor.json", "w");
