<?php
	...
	$stmt=$pdo->prepare("DELETE FROM bachelor_college_announcements WHERE announcement_id=?");
	$stmt->execute(array($announcementId));

	$stmt=$pdo->prepare("SELECT * FROM bachelor_college_announcements ORDER BY announcement_time DESC");
	$stmt->execute();
	$response=array();
